...
public class ImageOperator implements Runnable {
    ...
    public void run() {
        ...
        {
            VisionInit.cameraTab.add("High Power Port Alignment", outputStream).withWidget(BuiltInWidgets.kCameraStream)
        ...
        while(true){
            try{
                ...
                synchronized (VisionInit.tapeLock)
                {
                    ...
                    if (!VisionInit.isDistanceAngleFresh)
                    {
                       VisionInit.tapeLock.wait();
                    }

                    portDistance = VisionInit.tapeDistance;
                    angleToTurn = VisionInit.tapeAngle;
                    contourIndex = VisionInit.tapeContours;
                    isTargetFound = VisionInit.isTargetFound;
                    VisionInit.isDistanceAngleFresh = false; // these data captured to be processed so mark them as used
                }
